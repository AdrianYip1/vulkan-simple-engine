...
void TriangleApplication::createSwapChain() {
	...
	// Add one more than minimum since sticking to the min means that
	// sometimes will have to wait on the driver to complete internal operations
...
void TriangleApplication::createSyncObjects() {
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
		if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS ||
			...
			throw std::runtime_error("Failed to create sync devices");
		}
	}
...
void TriangleApplication::createVertexBuffer() {
	...
	createBuffer(bufferSize, stagingUsage, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
	...
	createBuffer(bufferSize, usage, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vertexBuffer, vertexBufferMemory);
...
void TriangleApplication::createIndexBuffer() {
	...
	createBuffer(bufferSize, usage, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, indexBuffer, indexBufferMemory);
...
void TriangleApplication::updateUniformBuffer(uint32_t currentImage) {
	...
	UniformBufferObject ubo{ enginemath::Mat4::identity(), enginemath::Mat4::identity(), enginemath::Mat4::identity() };
	enginemath::Mat4 _model = enginemath::Mat4::rotateZ(90 * sin(enginemath::toRad(time)));
	
	enginemath::Mat4 _view = enginemath::Mat4::lookAtM(enginemath::Vec3(2.0f, 2.0f, 2.0f), 
												       enginemath::Vec3(0.0f, 0.0f, 0.0f), 
													   enginemath::Vec3(0.0f, 1.0f, 0.0f));

	enginemath::Mat4 _proj = enginemath::Mat4::projectionM(enginemath::toRad(45.0f), aspect, 0.1f, 10.0f);
...
void TriangleApplication::createImage(uint32_t width, uint32_t height, VkFormat format,
...
void TriangleApplication::transitionImageLayout(VkImage image, VkFormat format,
...
VkFormat TriangleApplication::findSupportedFormat(const std::vector<VkFormat>& candidates,
